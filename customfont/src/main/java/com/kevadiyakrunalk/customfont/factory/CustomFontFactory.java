package com.kevadiyakrunalk.customfont.factory;
...
import com.kevadiyakrunalk.customfont.R;
import com.kevadiyakrunalk.customfont.CustomFontConfig;
import com.kevadiyakrunalk.customfont.HasTypeface;
import com.kevadiyakrunalk.customfont.util.CustomFontUtils;
import com.kevadiyakrunalk.customfont.util.ReflectionUtils;
import com.kevadiyakrunalk.customfont.util.TypefaceUtils;

import java.lang.ref.WeakReference;
import java.lang.reflect.Method;

/**
 * The type Custom font factory.
 */
public class CustomFontFactory {
    ...
    protected static int[] getStyleForTextView(TextView view) {
        ...
        if (styleIds[0] == -1) {
            ...
            styleIds[0] = CustomFontConfig.get().getClassStyles().containsKey(view.getClass())
                    ? CustomFontConfig.get().getClassStyles().get(view.getClass())
    ...
    /**
     * Parent is toolbar v 7 boolean.
     *
     * @param view the view
     * @return the boolean
     */
    protected static boolean parentIsToolbarV7(View view) {
        return CustomFontUtils.canCheckForV7Toolbar() && view.getParent() != null && (view.getParent() instanceof Toolbar);
    }
    ...
    /**
     * Instantiates a new Custom font factory.
     *
     * @param attributeId the attribute id
     */
    public CustomFontFactory(int attributeId) {
    ...
    public View onViewCreated(View view, Context context, AttributeSet attrs) {
        if (view != null && view.getTag(R.id.customfont_tag_id) != Boolean.TRUE) {
            onViewCreatedInternal(view, context, attrs);
            view.setTag(R.id.customfont_tag_id, Boolean.TRUE);
        }
        return view;
    }

    /**
     * On view created internal.
     *
     * @param view    the view
    ...
    void onViewCreatedInternal(View view, final Context context, AttributeSet attrs) {
        if (view instanceof TextView) {
            ...
            if (TextUtils.isEmpty(textViewFont)) {
                ...
                if (styleForTextView[1] != -1)
                    textViewFont = CustomFontUtils.pullFontPathFromTheme(context, styleForTextView[0], styleForTextView[1], mAttributeId);
                else
                    textViewFont = CustomFontUtils.pullFontPathFromTheme(context, styleForTextView[0], mAttributeId);
            }
            ...
            CustomFontUtils.applyFontToTextView(context, (TextView) view, CustomFontConfig.get(), textViewFont, deferred);
        }

        // AppCompat API21+ The ActionBar doesn't inflate default Title/SubTitle, we need to scan the
        // Toolbar(Which underlies the ActionBar) for its children.
        if (CustomFontUtils.canCheckForV7Toolbar() && view instanceof Toolbar) {
        ...
        } else if (CustomFontConfig.get().isCustomViewTypefaceSupport() && CustomFontConfig.get().isCustomViewHasTypeface(view)) {
    ...
    private Typeface getDefaultTypeface(Context context, String fontPath) {
        if (TextUtils.isEmpty(fontPath)) {
            fontPath = CustomFontConfig.get().getFontPath();
        }
        if (!TextUtils.isEmpty(fontPath)) {
    ...
    private String resolveFontPath(Context context, AttributeSet attrs) {
        ...
        String textViewFont = CustomFontUtils.pullFontPathFromView(context, attrs, mAttributeId);

        // Try view style attributes
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = CustomFontUtils.pullFontPathFromStyle(context, attrs, mAttributeId);
        }

        // Try View TextAppearance
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = CustomFontUtils.pullFontPathFromTextAppearance(context, attrs, mAttributeId);
        }
    ...
    private static class ToolbarLayoutListener implements ViewTreeObserver.OnGlobalLayoutListener {
        ...
        private final WeakReference<CustomFontFactory> mCalligraphyFactory;
        private final WeakReference<Context> mContextRef;
        private final WeakReference<Toolbar> mToolbarReference;
        private final CharSequence originalSubTitle;

        private ToolbarLayoutListener(final CustomFontFactory customFontFactory,
                                      final Context context, Toolbar toolbar) {
            mCalligraphyFactory = new WeakReference<>(customFontFactory);
        ...
        public void onGlobalLayout() {
            ...
            final CustomFontFactory factory = mCalligraphyFactory.get();
