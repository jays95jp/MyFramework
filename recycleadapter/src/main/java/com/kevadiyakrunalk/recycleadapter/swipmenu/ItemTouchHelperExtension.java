...
import com.kevadiyakrunalk.recycleadapter.R;
...
public class ItemTouchHelperExtension extends RecyclerView.ItemDecoration
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                switch (swipeDir) {
                    ...
                    case START:
                        targetTranslateY = 0;
                        targetTranslateX = Math.signum(mDx) * getSwipeWidth(START);
                        break;
                    case RIGHT:
                    case END:
                        targetTranslateY = 0;
                        targetTranslateX = Math.signum(mDx) * getSwipeWidth(END);
                        break;
                    case UP:
                    case DOWN:
                        targetTranslateX = 0;
    ...
    private float getSwipeWidth(int direction) {
        if (mSelected instanceof Extension) {
            if(direction == START)
                return ((Extension) mSelected).getStartContentWidth();
            else
                return ((Extension) mSelected).getEndContentWidth();
        }
