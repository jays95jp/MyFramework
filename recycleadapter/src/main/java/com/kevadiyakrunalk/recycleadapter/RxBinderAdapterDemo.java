 ...
 */
public class RxBinderAdapterDemo
        extends AbstractExpandableItemAdapter<RxBinderAdapterDemo.MyGroupViewHolder, RxBinderAdapterDemo.MyChildViewHolder>
        implements ExpandableDraggableItemAdapter<RxBinderAdapterDemo.MyGroupViewHolder, RxBinderAdapterDemo.MyChildViewHolder>,
        ExpandableSwipeableItemAdapter<RxBinderAdapterDemo.MyGroupViewHolder, RxBinderAdapterDemo.MyChildViewHolder>
{
    private static final String SAVED_STATE_EXPANDABLE_ITEM_MANAGER = "RecyclerViewExpandableItemManager";
    private Integer variable;
    private List<Pair<Object, List<Object>>> mProvider;

    private Map<Class<?>, Object> map;
    private PublishSubject<MyBaseViewHolder> mPublishSubject;
    ...
    public RxBinderAdapterDemo(List<Pair<Object, List<Object>>> list, Integer variable, Map<Class<?>, Object> map,
    ...
    public int getChildCount(int groupPosition) {
        return mProvider.get(groupPosition).second.size();
    }

    @Override
    public long getGroupId(int groupPosition) {
        return ((GroupData) mProvider.get(groupPosition).first).getGroupId();
    }

    @Override
    public long getChildId(int groupPosition, int childPosition) {
        return ((ChildData) mProvider.get(groupPosition).second.get(childPosition)).getChildId();
    }

    @Override
    public int getGroupItemViewType(int groupPosition) {
        if (layoutHandler != null)
            return layoutHandler.getItemLayout(new ItemPosition(mProvider.get(groupPosition).first, groupPosition, -1));
        else
            return ((int) map.get(mProvider.get(groupPosition).first.getClass()));
    }

    @Override
    public int getChildItemViewType(int groupPosition, int childPosition) {
        if (layoutHandler != null)
            return layoutHandler.getItemLayout(new ItemPosition(mProvider.get(groupPosition).second.get(childPosition), groupPosition, childPosition));
        else
            return ((int) map.get(mProvider.get(groupPosition).second.get(childPosition).getClass()));
    }

    @Override
    public MyGroupViewHolder onCreateGroupViewHolder(ViewGroup view, int type) {
        ViewDataBinding binding = DataBindingUtil.inflate(inflater, type, view, false);
        return new MyGroupViewHolder(binding);
    }

    @Override
    public MyChildViewHolder onCreateChildViewHolder(ViewGroup view, int type) {
        ViewDataBinding binding = DataBindingUtil.inflate(inflater, type, view, false);
    ...
    public void onBindGroupViewHolder(MyGroupViewHolder holder, int groupPosition, int viewType) {
        holder.bindTo(groupPosition, mProvider.get(groupPosition).first);
        mPublishSubject.onNext(holder);
    }

    @Override
    public void onBindChildViewHolder(MyChildViewHolder holder, int groupPosition, int childPosition, int viewType) {
        holder.bindTo(groupPosition, childPosition, mProvider.get(groupPosition).second.get(childPosition));
        mPublishSubject.onNext(holder);
    }

    @Override
    public void onBindGroupViewHolder(MyGroupViewHolder holder, int groupPosition, int viewType, List<Object> payloads) {
    ...
    @Override
    public void onBindChildViewHolder(MyChildViewHolder holder, int groupPosition, int childPosition, int viewType, List<Object> payloads) {
        if (isForDataBinding(payloads))
            holder.getBinding().executePendingBindings();
        else
    ...
    public boolean onCheckCanExpandOrCollapseGroup(MyGroupViewHolder holder, int groupPosition, int x, int y, boolean expand) {
        ...
        if(mProvider.get(groupPosition).first instanceof GroupData) {
            if (((GroupData) mProvider.get(groupPosition).first).isPinned() != DataUtils.SWAP_NONE) {
    ...
    public void onMoveGroupItem(int fromGroupPosition, int toGroupPosition) {
        ...
        final Pair<Object, List<Object>> item = mProvider.remove(fromGroupPosition);
    ...
    public void onMoveChildItem(int fromGroupPosition, int fromChildPosition, int toGroupPosition, int toChildPosition) {
        ...
        final Pair<Object, List<Object>> fromGroup = mProvider.get(fromGroupPosition);
        final Pair<Object, List<Object>> toGroup = mProvider.get(toGroupPosition);

        final Object item = fromGroup.second.remove(fromChildPosition);
    ...
    public SwipeResultAction onSwipeGroupItem(MyGroupViewHolder holder, int groupPosition, int result) {
        switch (result) {
            ...
            case Swipeable.RESULT_SWIPED_RIGHT:
                if(mProvider.get(groupPosition).first instanceof GroupData) {
                    if(((GroupData) mProvider.get(groupPosition).first).isPinned() == DataUtils.SWAP_LEFT) {
            ...
            case Swipeable.RESULT_SWIPED_LEFT:
                if(mProvider.get(groupPosition).first instanceof GroupData) {
                    if(((GroupData) mProvider.get(groupPosition).first).isPinned() == DataUtils.SWAP_RIGHT) {
                        // pinned --- back to default position
                        return new GroupUnpinResultAction(this, groupPosition);
                    } else {
                        // not pinned --- remove
    ...
    public SwipeResultAction onSwipeChildItem(MyChildViewHolder holder, int groupPosition, int childPosition, int result) {
        switch (result) {
            // swipe right
            case Swipeable.RESULT_SWIPED_RIGHT:
                if(mProvider.get(groupPosition).second.get(childPosition) instanceof ChildData) {
                    if (((ChildData) mProvider.get(groupPosition).second.get(childPosition)).isPinned() == DataUtils.SWAP_LEFT) {
                        // pinned --- back to default position
            ...
            case Swipeable.RESULT_SWIPED_LEFT:
                if(mProvider.get(groupPosition).second.get(childPosition) instanceof ChildData) {
                    if (((ChildData) mProvider.get(groupPosition).second.get(childPosition)).isPinned() == DataUtils.SWAP_RIGHT) {
                        // pinned --- back to default position
                        return new ChildUnpinResultAction(this, groupPosition, childPosition);
                    } else {
                        // not pinned --- remove
    ...
    private static class GroupSwipeLeftResultAction extends SwipeResultActionMoveToSwipedDirection {
        ...
        GroupSwipeLeftResultAction(RxBinderAdapterDemo adapter, int groupPosition) {
            mAdapter = adapter;
        ...
        protected void onPerformAction() {
            ...
            if(mAdapter.mProvider.get(mGroupPosition).first instanceof GroupData) {
                GroupData item = (GroupData) mAdapter.mProvider.get(mGroupPosition).first;
    ...
    private static class GroupSwipeRightResultAction extends SwipeResultActionRemoveItem {
        ...
        GroupSwipeRightResultAction(RxBinderAdapterDemo adapter, int groupPosition) {
            mAdapter = adapter;
            mGroupPosition = groupPosition;
        }
    ...
    private static class GroupUnpinResultAction extends SwipeResultActionDefault {
        ...
        GroupUnpinResultAction(RxBinderAdapterDemo adapter, int groupPosition) {
            mAdapter = adapter;
            mGroupPosition = groupPosition;
        }
    ...
    private static class ChildSwipeLeftResultAction extends SwipeResultActionMoveToSwipedDirection {
        ...
        ChildSwipeLeftResultAction(RxBinderAdapterDemo adapter, int groupPosition, int childPosition) {
            mAdapter = adapter;
            mGroupPosition = groupPosition;
        ...
        protected void onPerformAction() {
            ...
            if(mAdapter.mProvider.get(mGroupPosition).second.get(mChildPosition) instanceof ChildData) {
                ChildData item = (ChildData) mAdapter.mProvider.get(mGroupPosition).second.get(mChildPosition);
    ...
    private static class ChildSwipeRightResultAction extends SwipeResultActionRemoveItem {
        ...
        ChildSwipeRightResultAction(RxBinderAdapterDemo adapter, int groupPosition, int childPosition) {
            mAdapter = adapter;
            mGroupPosition = groupPosition;
            mChildPosition = childPosition;
        }
    ...
    private static class ChildUnpinResultAction extends SwipeResultActionDefault {
        ...
        ChildUnpinResultAction(RxBinderAdapterDemo adapter, int groupPosition, int childPosition) {
            mAdapter = adapter;
            mGroupPosition = groupPosition;
            mChildPosition = childPosition;
        }
    ...
    public rx.Observable<MyBaseViewHolder> asObservable() {
        return mPublishSubject.asObservable();
    }

    public List<Pair<Object, List<Object>>> getDataSet() {
        return mProvider;
    }

    public void updateDataSet(List<Pair<Object, List<Object>>> dataSet) {
        mProvider = dataSet;
        notifyDataSetChanged();
    }

    public static RxBinderAdapterDemo.Builder with(List<Pair<Object, List<Object>>> list, int variable) {
        return (new RxBinderAdapterDemo.Builder(list, variable));
    }

    public static class Builder implements RecyclerViewExpandableItemManager.OnGroupExpandListener, RecyclerViewExpandableItemManager.OnGroupCollapseListener {
        private Integer variable;
        private List<Pair<Object, List<Object>>> list;
        private Integer[] onClickResIds;
        private Integer[] onLongClickResIds;

        private Map<Class<?>, Object> map;
        private RxBinderAdapterDemo.LayoutHandler handler = null;
        private RxBinderAdapterDemo.OnClickListener onClick = null;
        private RxBinderAdapterDemo.OnLoadMoreListener onLoadMore = null;
        private RxBinderAdapterDemo.OnLongClickListener onLongClick = null;

        private Integer contentResId, expandIndicatorResId, dragHandleResId;
        private Integer childItemHeight, topMargin, bottomMargin;
        private Float intMaxLeftSwipeAmount = 0f, intMaxRightSwipeAmount = 0f;

        public RxBinderAdapterDemo.Builder map(Class<?> clazz, int layout) {
            map.put(clazz, layout);
            return this;
        }

        public <T> RxBinderAdapterDemo.Builder map(int layout) {
            return map(Object.class, layout);
        }

        public RxBinderAdapterDemo.Builder layoutHandler(RxBinderAdapterDemo.LayoutHandler layoutHandler) {
            handler = layoutHandler;
            return this;
        }

        public RxBinderAdapterDemo.Builder layout(Func1<? super RxBinderAdapterDemo.ItemPosition, Integer> func1) {
            return layoutHandler((detail) -> func1.call(detail).intValue());
        }

        public RxBinderAdapterDemo.Builder onClickListener(RxBinderAdapterDemo.OnClickListener listener, Integer... resIds) {
            onClick = listener;
            onClickResIds = resIds;
            return this;
        }

        public RxBinderAdapterDemo.Builder onClick(Func1<? super RxBinderAdapterDemo.ItemViewTypePosition, Void> func1, Integer... resIds) {
            return onClickListener((detail) -> func1.call(detail), resIds);
        }

        public RxBinderAdapterDemo.Builder onLongClickListener(RxBinderAdapterDemo.OnLongClickListener listener, Integer... resIds) {
            onLongClick = listener;
            onLongClickResIds = resIds;
            return this;
        }

        public RxBinderAdapterDemo.Builder onLongClick(Func1<? super RxBinderAdapterDemo.ItemViewTypePosition, Void> func1, Integer... resIds) {
            return onLongClickListener((detail) -> func1.call(detail), resIds);
        }

        public RxBinderAdapterDemo.Builder onLoadMoreListener(RxBinderAdapterDemo.OnLoadMoreListener listener) {
            onLoadMore = listener;
            return this;
        }

        public RxBinderAdapterDemo.Builder onLoadMore(Func1<? super Integer, Boolean> func1) {
            return onLoadMoreListener(size -> func1.call(size));
        }

        public RxBinderAdapterDemo.Builder onSwipMenuListener(Integer contentViewResId, Float intMaxLeftSwipeAmount, Float intMaxRightSwipeAmount) {
        ...
        public Builder onExpandListener(Integer expandIndicatorResId, Integer childItemHeight, Integer topMargin, Integer bottomMargin, Bundle savedInstanceState) {
            this.expandIndicatorResId = expandIndicatorResId;
        ...
        public RxBinderAdapterDemo into(RecyclerView recyclerView, RecyclerView.LayoutManager... layoutManager) {
            ...
            RxBinderAdapterDemo adapter = build();
        ...
        public RxBinderAdapterDemo build() {
            return new RxBinderAdapterDemo(
        ...
        public Builder(List<Pair<Object, List<Object>>> list, Integer variable) {
            super();
            this.list = list;
            this.variable = variable;
            map = new HashMap<>();
